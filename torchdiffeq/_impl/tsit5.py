...
class Tsit5Solver(AdaptiveStepsizeODESolver):
    ...
    def before_integrate(self, t):
        ...
        if self.first_step is None:
            first_step = _select_initial_step(self.func, t0, self.y0, 4, self.rtol, self.atol).to(t)
        else:
            first_step = _convert_to_tensor(0.01, dtype=t.dtype, device=t.device)
        self.rk_state = _RungeKuttaState(
            self.y0,
            self.func(t[0].type_as(self.y0[0]), self.y0), t0, t0, first_step,
    ...
    def advance(self, next_t):
        ...
        while increasing_check(next_t > self.rk_state.t1):
            ...
            self.rk_state = self._adaptive_tsit5_step(self.rk_state, mask)
            n_steps += 1
        return _interp_eval_tsit5(self.rk_state.t0, self.rk_state.t1, self.rk_state.interp_coeff, next_t)

    def _adaptive_tsit5_step(self, rk_state, mask=None):
        """Take an adaptive Runge-Kutta step to integrate the ODE."""
        y0, f0, t0_prev, t0, dt, _ = rk_state
        ########################################################
        #                      Assertions                      #
        ########################################################
        if t0.shape[0] > 1:
